...
void font_draw(font *f, app *a, const char *text, v2f pos, uint color, int shadow,
               float scale) {
  arr_clear(f->vs);
  float dx = pos.x;
  ...
  for (char const *c = text; *c; c++) {
    ...
    if (shadow) {
      ...
      arr_add(&f->vs, &(font_vtx){.pos = {dxs + 1, dys + 1, 1}, .uv = {(float) pc.x0 / 2048.f, (float) pc.y0 / 2048.f}, .col = col});
      arr_add(&f->vs, &(font_vtx){.pos = {dx1s + 1, dys + 1, 1}, .uv = {(float) pc.x1 / 2048.f, (float) pc.y0 / 2048.f}, .col = col});
      arr_add(&f->vs, &(font_vtx){.pos = {dx1s + 1, dy1s + 1, 1}, .uv = {(float) pc.x1 / 2048.f, (float) pc.y1 / 2048.f}, .col = col});
      arr_add(&f->vs, &(font_vtx){.pos = {dx1s + 1, dy1s + 1, 1}, .uv = {(float) pc.x1 / 2048.f, (float) pc.y1 / 2048.f}, .col = col});
      arr_add(&f->vs, &(font_vtx){.pos = {dxs + 1, dy1s + 1, 1}, .uv = {(float) pc.x0 / 2048.f, (float) pc.y1 / 2048.f}, .col = col});
      arr_add(&f->vs, &(font_vtx){.pos = {dxs + 1, dys + 1, 1}, .uv = {(float) pc.x0 / 2048.f, (float) pc.y0 / 2048.f}, .col = col});
    }

    col = (v4f){r, g, b, alpha};
    arr_add(&f->vs, &(font_vtx){.pos = {dxs, dys, 1}, .uv = {(float) pc.x0 / 2048.f, (float) pc.y0 / 2048.f}, .col = col});
    arr_add(&f->vs, &(font_vtx){.pos = {dx1s, dys, 1}, .uv = {(float) pc.x1 / 2048.f, (float) pc.y0 / 2048.f}, .col = col});
    arr_add(&f->vs, &(font_vtx){.pos = {dx1s, dy1s, 1}, .uv = {(float) pc.x1 / 2048.f, (float) pc.y1 / 2048.f}, .col = col});
    arr_add(&f->vs, &(font_vtx){.pos = {dx1s, dy1s, 1}, .uv = {(float) pc.x1 / 2048.f, (float) pc.y1 / 2048.f}, .col = col});
    arr_add(&f->vs, &(font_vtx){.pos = {dxs, dy1s, 1}, .uv = {(float) pc.x0 / 2048.f, (float) pc.y1 / 2048.f}, .col = col});
    arr_add(&f->vs, &(font_vtx){.pos = {dxs, dys, 1}, .uv = {(float) pc.x0 / 2048.f, (float) pc.y0 / 2048.f}, .col = col});
